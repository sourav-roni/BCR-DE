'''
BCR-DE model file where noise is added to 
detail coefficient in order to regularize
'''
import torch
import numpy as np
import pdb
...
class myDense(nn.Module):
	'''
	Dense layer via einsum in multiple dimension
	'''
...
class CDE_BCR_noise(nn.Module):
	'''
	BCR_DE model with noise addition in the detail, this can improve performance by regularization
	'''
	def __init__(self, time_step, wave, D, D_out, d, k, original_length, num_classes, nonlinearity, n_levels, K_dense, K_LC, nb, num_sparse_LC, use_cheap_sparse_LC, interpol, conv_bias, predict=False, masked_modelling=False):
		super(CDE_BCR_noise, self).__init__()
	...
	def forward(self, seq, coeffs, time, is_inference=False):
		...
		for l in range(self.n_levels):
			...
			if is_inference:
				...
				cd_no_noise = current_detail[0]
				ca_no_noise = current_approx

				cd_scale = 0.01*torch.max(cd_no_noise)
				ca_scale = 0.01*torch.max(ca_no_noise)

				cd_noise = cd_no_noise + cd_scale*torch.randn(cd_no_noise.shape).to(cd_no_noise.device)
				ca_noise = ca_no_noise + ca_scale*torch.randn(ca_no_noise.shape).to(ca_no_noise.device)

				all_detail.append(cd_noise)
				all_approx.append(ca_noise)

...
def main():
	...
	for epoch in range(args.epoch):
		...
		for x, coeffs, y_true, time in tqdm(train_dl, leave=False):
			...
			y_true = jnp.astype(y_true, jnp.int32)

			graphdef, state = nnx.split((model, optimizer))
			y_pred = inference_step_jit(graphdef, state, x, coeffs, time_step)
			...
			pred = jnp.expand_dims(jnp.argmax(y_pred, axis=1), axis=1)
			y_true_reshaped = jnp.reshape(y_true, pred.shape)
			correct += int(jnp.sum(pred == y_true_reshaped))
		...
		for x, coeffs, y_true, time in tqdm(valid_dl, leave=False):
			...
			graphdef, state = nnx.split((model, optimizer))
			y_pred = inference_step_jit(graphdef, state, x, coeffs, time_step)
			loss = optax.losses.softmax_cross_entropy_with_integer_labels(y_pred, y_true).mean()
